...
# Deploy Kubernetes MCP Server with MCP Shield Sidecar on OpenShift
# This script automates the deployment process including OAuth client creation
#
# Usage:
#   ./deploy-openshift-kubernetes-mcp-server-sidecar.sh                    # Deploy with default settings
#   ./deploy-openshift-kubernetes-mcp-server-sidecar.sh --namespace mcp    # Deploy to specific namespace
#   ./deploy-openshift-kubernetes-mcp-server-sidecar.sh --cleanup          # Clean up deployment
#   ./deploy-openshift-kubernetes-mcp-server-sidecar.sh --help             # Show help
...
replace_placeholders() {
    ...
    sed "s/PLACEHOLDER/$CLUSTER_DOMAIN/g" openshift-kubernetes-mcp-server-sidecar.yml > /tmp/openshift-kubernetes-mcp-server-sidecar.yml
    
    # Replace service name and namespace if needed
    if [ "$NAMESPACE" != "default" ] || [ "$SERVICE_NAME" != "kubernetes-mcp-server" ]; then
        sed -i "s/namespace: default/namespace: $NAMESPACE/g" /tmp/openshift-kubernetes-mcp-server-sidecar.yml
        sed -i "s/name: kubernetes-mcp-server/name: $SERVICE_NAME/g" /tmp/openshift-kubernetes-mcp-server-sidecar.yml
        sed -i "s/app: kubernetes-mcp-server/app: $SERVICE_NAME/g" /tmp/openshift-kubernetes-mcp-server-sidecar.yml
    fi
...
deploy_application() {
    ...
    oc apply -f /tmp/openshift-kubernetes-mcp-server-sidecar.yml
...
cleanup() {
    ...
    rm -f /tmp/openshift-kubernetes-mcp-server-sidecar.yml
}
...
check_prerequisites() {
    ...
    if [ ! -f "openshift-kubernetes-mcp-server-sidecar.yml" ]; then
        print_error "openshift-kubernetes-mcp-server-sidecar.yml not found in current directory."
        print_status "Please run this script from the examples directory:"
        print_status "  cd examples && ./deploy-openshift-kubernetes-mcp-server-sidecar.sh"
        exit 1
    fi
...
main() {
    print_status "Starting Kubernetes MCP Server with MCP Shield sidecar deployment..."
    print_status ""
    ...
    print_status "4. Note: All Kubernetes operations use the user's OAuth token, not ServiceAccount permissions"
}
