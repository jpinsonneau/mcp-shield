...
# Deploy Loki MCP Server with MCP Shield Sidecar on OpenShift
# This script automates the deployment process including OAuth client creation
#
# Usage:
#   ./deploy-openshift-loki-sidecar.sh                    # Deploy with default settings
#   ./deploy-openshift-loki-sidecar.sh --namespace mcp    # Deploy to specific namespace
#   ./deploy-openshift-loki-sidecar.sh --cleanup          # Clean up deployment
#   ./deploy-openshift-loki-sidecar.sh --help             # Show help
...
# Default values
NAMESPACE="default"
OAUTH_CLIENT_ID="loki-mcp-server"
SERVICE_NAME="loki-mcp-server"
...
replace_placeholders() {
    ...
    sed "s/PLACEHOLDER/$CLUSTER_DOMAIN/g" openshift-loki-sidecar.yml > /tmp/openshift-loki-sidecar.yml
    
    # Replace service name and namespace if needed
    if [ "$NAMESPACE" != "default" ] || [ "$SERVICE_NAME" != "loki-mcp-server" ]; then
        sed -i "s/namespace: default/namespace: $NAMESPACE/g" /tmp/openshift-loki-sidecar.yml
        sed -i "s/name: loki-mcp-server/name: $SERVICE_NAME/g" /tmp/openshift-loki-sidecar.yml
        sed -i "s/app: loki-mcp-server/app: $SERVICE_NAME/g" /tmp/openshift-loki-sidecar.yml
    fi
    
    # Replace MCP Shield image if custom image provided
    if [ "$MCP_SHIELD_IMAGE" != "quay.io/jpinsonn/mcp-shield:dev" ]; then
        sed -i "s|quay.io/jpinsonn/mcp-shield:dev|$MCP_SHIELD_IMAGE|g" /tmp/openshift-loki-sidecar.yml
    fi
...
deploy_application() {
    ...
    oc apply -f /tmp/openshift-loki-sidecar.yml
...
cleanup() {
    ...
    rm -f /tmp/openshift-loki-sidecar.yml
}
...
cleanup_deployment() {
    ...
    # Delete secret
    oc delete secret "${SERVICE_NAME}-auth" -n "$NAMESPACE" --ignore-not-found=true 2>/dev/null || true
...
check_prerequisites() {
    ...
    if [ ! -f "openshift-loki-sidecar.yml" ]; then
        print_error "openshift-loki-sidecar.yml not found in current directory."
        print_status "Please run this script from the examples directory:"
        print_status "  cd examples && ./deploy-openshift-loki-sidecar.sh"
        exit 1
    fi
...
main() {
    print_status "Starting Loki MCP Server with MCP Shield sidecar deployment..."
    print_status ""
    ...
    print_status ""
    print_status "Note: Make sure to configure LOKI_URL environment variable in the loki-mcp-server container"
    print_status "      to point to your Loki instance. Update the deployment if needed:"
    print_status "      oc set env deployment/$SERVICE_NAME -c loki-mcp-server -n $NAMESPACE LOKI_URL=<your-loki-url>"
}
...
Options:
  ...
  --client-id ID            OAuth client ID (default: loki-mcp-server)
  --service-name NAME       Service name (default: loki-mcp-server)
...
Examples:
  ...
  $0 --client-id my-loki-mcp-server     # Use custom OAuth client ID
