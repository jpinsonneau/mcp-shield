...
# Deploy MCP Gateway with MCP Shield Sidecar on OpenShift
# This script automates the deployment process including OAuth client creation
#
# Usage:
#   ./deploy-openshift-mcp-gateway-sidecar.sh                    # Deploy with default settings
#   ./deploy-openshift-mcp-gateway-sidecar.sh --namespace mcp-system    # Deploy to specific namespace
#   ./deploy-openshift-mcp-gateway-sidecar.sh --cleanup          # Clean up deployment
#   ./deploy-openshift-mcp-gateway-sidecar.sh --help             # Show help
...
replace_placeholders() {
    ...
    sed "s/PLACEHOLDER/$CLUSTER_DOMAIN/g" openshift-mcp-gateway-sidecar.yml > /tmp/openshift-mcp-gateway-sidecar.yml
    
    # Replace service name and namespace if needed
    if [ "$NAMESPACE" != "default" ] || [ "$SERVICE_NAME" != "mcp-gateway" ]; then
        sed -i "s/namespace: default/namespace: $NAMESPACE/g" /tmp/openshift-mcp-gateway-sidecar.yml
        sed -i "s/name: mcp-gateway/name: $SERVICE_NAME/g" /tmp/openshift-mcp-gateway-sidecar.yml
        sed -i "s/app: mcp-gateway/app: $SERVICE_NAME/g" /tmp/openshift-mcp-gateway-sidecar.yml
    fi
    
    # Replace MCP Shield image if custom image provided
    if [ "$MCP_SHIELD_IMAGE" != "quay.io/jpinsonn/mcp-shield:dev" ]; then
        sed -i "s|quay.io/jpinsonn/mcp-shield:dev|$MCP_SHIELD_IMAGE|g" /tmp/openshift-mcp-gateway-sidecar.yml
    fi
...
deploy_application() {
    ...
    oc apply -f /tmp/openshift-mcp-gateway-sidecar.yml
...
get_deployment_info() {
    ...
    if [ -n "$ROUTE_URL" ]; then
        ...
        print_status ""
        print_status "Note: MCP Gateway aggregates tools from multiple backend MCP servers."
        print_status "Configure backend servers in the mcp-gateway-config ConfigMap."
        print_status "═══════════════════════════════════════════════════════════"
    else
...
cleanup() {
    ...
    rm -f /tmp/openshift-mcp-gateway-sidecar.yml
}
...
check_prerequisites() {
    ...
    if [ ! -f "openshift-mcp-gateway-sidecar.yml" ]; then
        print_error "openshift-mcp-gateway-sidecar.yml not found in current directory."
        print_status "Please run this script from the examples directory:"
        print_status "  cd examples && ./deploy-openshift-mcp-gateway-sidecar.sh"
        exit 1
    fi
...
main() {
    print_status "Starting MCP Gateway with MCP Shield sidecar deployment..."
    print_status ""
    ...
    print_status "4. Configure backend MCP servers in the mcp-gateway-config ConfigMap"
}
...
Options:
  ...
  --client-id ID            OAuth client ID (default: mcp-gateway)
  --service-name NAME       Service name (default: mcp-gateway)
...
Examples:
  ...
  $0 --namespace mcp-system             # Deploy to mcp-system namespace
  $0 --client-id my-mcp-gateway         # Use custom OAuth client ID
